...
export const useUserStore = defineStore('user', () => {
  ...
  const avatar = computed(() => userInfo.value?.shopAccount?.avatar)
  const nickname = computed(() => userInfo.value?.shopAccount?.nickname ?? userInfo.value?.shopAccount?.username)
  const roles = computed(() => userInfo.value?.shopAccount?.role)
